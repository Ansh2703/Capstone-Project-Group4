...
@dlt.table(
    name="dim_region",
    comment="Region reference dimension. Sourced from silver.regions (SCD-1, no history).",
    table_properties={
        "layer":        "gold",
        "domain":       "reference",
        "contains_pii": "false",
...
@dlt.table(
    ...
    comment=(
        ...
        "Region attributes denormalised from silver.regions."
    ),
    table_properties={
        "layer":        "gold",
        "domain":       "stores",
...
@dlt.table(
    ...
    comment=(
        ...
        "See governance_udf_cls_rls.sql for masking UDF definitions and ALTER TABLE statements."
    ),
    table_properties={
        "layer":        "gold",
        "domain":       "customers",
...
@dlt.table(
    name="dim_product",
    comment="Conformed product dimension — current SCD-2 snapshot (__END_AT IS NULL).",
    table_properties={
        "layer":        "gold",
        "domain":       "products",
        "contains_pii": "false",
...
@dlt.table(
    ...
    comment="Executive overview containing Total Revenue and Profit Margin by Year/Month.",
...
@dlt.table(
    ...
    comment="Inventory alerts aggregation for real-time operations dashboard.",
...
@dlt.table(
    ...
    comment="Orders per minute aggregation for operations dashboard.",
    table_properties={"layer": "gold_agg", "domain": "ops_dashboard"}
)
...
@dlt.table(
    ...
    comment="Customer Lifetime Value (LTV) combining total spend, profit, and demographic profile.",
...
@dlt.table(
    ...
    comment="Retail operations metric calculating revenue generated per square foot of store space.",
    table_properties={"layer": "gold_agg", "domain": "ops_dashboard"}
)
